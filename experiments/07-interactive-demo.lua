...
function _G.exp07.extract_range(src, s, e)
  ...
  -- universe-path 命名：%%=range buffer, 携带源buffer信息
...
vim.api.nvim_buf_set_lines(src, 0, -1, false, source_lines)
vim.api.nvim_buf_set_name(src, "%")
_G.exp07.record_state(src)
...
local rbuf = _G.exp07.extract_range(src, 1, 3)
local rname = vim.api.nvim_buf_get_name(rbuf)
...
print("修改后区间内容:")
for _, line in ipairs(vim.api.nvim_buf_get_lines(rbuf, 0, -1, false)) do
  print("  " .. line)
end
...
local ok, err = _G.exp07.writeback(rbuf)
print(string.format("写回结果: %s", ok and "SUCCESS" or ("FAIL: " .. err)))
print("原文件buffer更新后内容:")
for _, line in ipairs(vim.api.nvim_buf_get_lines(src, 0, -1, false)) do
  print("  " .. line)
end
...
_G.exp07.record_state(src)
local rbuf2 = _G.exp07.extract_range(src, 4, 5)
vim.api.nvim_buf_set_lines(src, 0, 0, false, { "// external mod" })
ok, err = _G.exp07.writeback(rbuf2)
print(string.format("写回结果: %s", ok and "SUCCESS (unexpected!)" or ("REJECTED: " .. err)))

print("")
print("=== 实验07验证完成 ===")
print("buffer列表请用 :buffers 查看")
